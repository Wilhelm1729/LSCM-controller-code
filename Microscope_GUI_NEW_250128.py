...
class ScanTab:
    ...
    def init_fill_tabs(self, tabname="New Scan"):
        ...
        self.set_scope_length_widget(live_tab_1).grid(row=11, column=0, columnspan=2, sticky="news", padx=2, pady=0)
        self.set_stepsize_widget(live_tab_1).grid(row=12, column=0, columnspan=2, sticky="news", padx=2, pady=0)
        self.heatmap_ratio_widget(live_tab_1).grid(row=13, column=0, columnspan=2, sticky="news", padx=2, pady=0)
        # Logged information:
    ...
    def heatmap_ratio_widget(self, tab):
        ...
        def findpeaks():
            ...
            matrix, results = peak_analysis.process_matrix_universalthresh(matrix2)
        ...
        def g2_measrement_peaks():
            ...
            for res in results:
                x, y = res['position']
                self.t7.move_to_pos(x_coord=x, y_coord=y)
                ...
                self.t7.filename = self.t7.filename.replace('date(',  f"coordinate({x},{y})_" + 'date(')
                self.t7.filename = self.t7.filename.replace('/Data/', '/g2_Data/')
                print(f"filename is {self.t7.filename}")
                self.t7.socket_connection2(g2=True)
                time.sleep(20) #TODO: the sleep time can not be auto adjusted with dump time. maybe we should include dumptime into the filename
        ...
        ttk.Label(frm_ratio, text=f'find brightest 10 spots').grid(row=2, column=0,columnspan=2, sticky="ew")
        find_peaks_button = ttk.Button(frm_ratio, text="find peaks2", command = findpeaks )
        find_peaks_button.grid(row=2, column=1, sticky="ew", padx=1, pady=1)
        g2_measurement_button = ttk.Button(frm_ratio, text="g2_measurement", command = g2_measrement_peaks )
        g2_measurement_button.grid(row=3, column=1, sticky="ew", padx=1, pady=1)
...
class T7:
    ...
    def __init__(self, master):
        ...
        self.x_address = "DAC1"   # Values sent from periodic buffer (which is not compatible with TDAC) this is Xaxis which does sweep (DAC1)
    ...
    def slow_galvo_scan(self, do_scan=True):
        ...
        def start_pos_si():
            ...
            if not self.offline:   #****
                ...
                ljm.eWriteNames(self.handle, len(add_list_001), add_list_001, val_list_001)  # sleep for 0.1s after step
        ...
        def check_cmds(addresses, values):
            ...
            for i in range(len(addresses)):
                ...
                # CHECK SWEEP CMDS
                    ...
                    if not (0 < values[i] < 4):
                        self.logger_box.module_logger.info("ERROR. SWEEP VALUE NOT IN RANGE (0, 4) V")
                        self.abort_scan = True
        ...
        try:
            ...
            add_list_001, val_list_001 = add_wait_delay_si(0.001)   # addresses and values for 0.01s delay waiting for galvo to move to pixel
    ...
    def socket_connection2(self, shutdown_server=False, doneScan=False, g2=False):
        """ Sets up a server ot communciate to the qutag computer to start a measurement
            Sends the file and scan time to the computer"""
