...
class ScanTab:
    ...
    def __init__(self, tab, tabname, new=True):
        ...
        ttk.Button(tab, text="Destroy Tab", command=self.destroy_tab).grid(
            row=50, column=0, sticky="w"
        )  # FOR ADDING MORE TABS
    ...
    def init_parameters(self):
        ...
        self.anal_data_file = tk.StringVar(
            value="/Users/juliawollter/Desktop/GUI Micro/Data/240918/hBN1000_scantime(100.0)_dwellTime(0.01)_sineAmp(0.2)_stepAmp(0.2)_stepDim(100)_date(240918)_time(15h14m56s).timeres"
        )
        ...
        self.minX = tk.DoubleVar(value=-0.3)
        self.maxX = tk.DoubleVar(value=0.3)
        self.minY = tk.DoubleVar(value=-0.3)
        self.maxY = tk.DoubleVar(value=0.3)
    ...
    def init_fill_tabs(self, tabname="New Scan"):
        ...
        scan_conf.grid(row=1, column=0, columnspan=2, sticky="news", padx=2, pady=0)
        ...
        self.plot_analysis_widget(live_tab_2).grid(
            row=0, column=2, sticky="", padx=2, pady=0
        )  # in sub frame
    ...
    def add_to_grid(widg, rows, cols, sticky, columnspan=None):
        for i in range(len(widg)):
            if columnspan:
                ...
                widg[i].grid(
                    row=rows[i], column=cols[i], sticky=sticky[i], padx=0, pady=0
                )
    ...
    def suggest_name(self):
        ...
        self.variable_containers["scantime title"]["lab"].config(
            text=f"Approx scan time = {scantime} sec ({round(scantime / 60, 1)} min)"
        )
    ...
    def choose_scan_configs_widget(self, tab):
        ...
        def update_scan_range(key, val):
            ...
            if key == "amp X":
                self.minX.set(-eval(val))  # +self.t7.x_offset)
                self.maxX.set(eval(val))  # +self.t7.x_offset)

            elif key == "amp Y":
                self.minY.set(-eval(val))  # +self.t7.y_offset)
                self.maxY.set(eval(val))  # +self.t7.y_offset)

            elif key == "min X" or key == "max X":
                print(self.t7.prev_speed_mode)
                self.ampX.set((self.maxX.get() - self.minX.get()) / 2)

            elif key == "min Y" or key == "max Y":
                print(self.t7.prev_speed_mode)
                self.ampY.set((self.maxY.get() - self.minY.get()) / 2)
            else:
        ...
        theo_scan_lbl.grid(row=0, column=1, columnspan=10, sticky="ew", padx=0, pady=0)
        ...
        variables = self.variable_containers = {
            "info": {
                ...
                "entry": ttk.Entry(frm_misc, textvariable=self.clue, width=10),
                "var": self.clue,
            },
        ...
        variables["info"]["lab"].grid(row=i, column=0, sticky="ew", padx=0, pady=0)
        variables["info"]["entry"].grid(
            row=i, column=1, columnspan=10, sticky="ew", padx=0, pady=0
        )

        variables["int time"]["lab"].grid(
            row=i + 1, column=0, sticky="ew", padx=0, pady=0
        )
        variables["int time"]["entry"].grid(
            row=i + 1, column=1, sticky="w", padx=0, pady=0
        )

        variables["dim Y"]["lab"].grid(row=i + 2, column=0, sticky="ew", padx=0, pady=0)
        variables["dim Y"]["entry"].grid(
            row=i + 2, column=1, sticky="w", padx=0, pady=0
        )

        variables["amp X"]["lab"].grid(row=i + 1, column=2, sticky="w", padx=0, pady=0)
        variables["amp X"]["entry"].grid(
            row=i + 1, column=3, sticky="w", padx=0, pady=0
        )
        variables["amp Y"]["lab"].grid(row=i + 2, column=2, sticky="w", padx=0, pady=0)
        variables["amp Y"]["entry"].grid(
            row=i + 2, column=3, sticky="w", padx=0, pady=0
        )

        variables["min X"]["lab"].grid(row=i + 1, column=4, sticky="w", padx=0, pady=0)
        variables["min X"]["entry"].grid(
            row=i + 1, column=5, sticky="w", padx=0, pady=0
        )
        variables["min Y"]["lab"].grid(row=i + 2, column=4, sticky="w", padx=0, pady=0)
        variables["min Y"]["entry"].grid(
            row=i + 2, column=5, sticky="w", padx=0, pady=0
        )

        variables["max X"]["lab"].grid(row=i + 1, column=6, sticky="w", padx=0, pady=0)
        variables["max X"]["entry"].grid(
            row=i + 1, column=7, sticky="w", padx=0, pady=0
        )
        variables["max Y"]["lab"].grid(row=i + 2, column=6, sticky="w", padx=0, pady=0)
        variables["max Y"]["entry"].grid(
            row=i + 2, column=7, sticky="w", padx=0, pady=0
        )

        variables["filename"]["butt"].grid(
            row=i + 3, column=0, sticky="ew", padx=0, pady=0
        )
        variables["filename"]["entry"].grid(
            row=i + 3, column=1, columnspan=10, sticky="ew", padx=0, pady=0
        )

        variables["data folder"]["butt"].grid(
            row=i + 4, column=0, sticky="ew", padx=0, pady=0
        )
        variables["data folder"]["entry"].grid(
            row=i + 4, column=1, columnspan=10, sticky="ew", padx=0, pady=0
        )
        ...
        variables["run scan check"]["butt"].grid(
            row=10, column=0, sticky="ew", padx=0, pady=0
        )
        ...
        variables["offline check"]["butt"].grid(
            row=11, column=0, sticky="ew", padx=0, pady=0
        )
    ...
    def choose_analysis_configs_widget(self, tab):
        ...
        ttk.Label(frm_misc, text="Analysis", font=("", 15)).grid(
            row=0, column=0, sticky="news", padx=0, pady=0
        )
        ...
        variables = {
            "file_name": {
                ...
                "entry": ttk.Entry(
                    frm_misc, textvariable=self.anal_data_file, width=15
                ),
                "var": self.save_folder,
            },
            "eta_recipe": {
                "butt": ttk.Button(frm_misc, text="eta recipe", command=get_recipe),
                "entry": ttk.Entry(frm_misc, textvariable=self.eta_recipe, width=30),
                "var": self.eta_recipe,
            },
            "save_folder": {
                "butt": ttk.Button(frm_misc, text="save folder", command=open_folder),
                "entry": ttk.Entry(frm_misc, textvariable=self.save_folder, width=15),
                "var": self.save_folder,
            },
            "bins": {
        ...
        for i, key in enumerate(variables.keys()):
            ...
            self.add_to_grid(
                widg=[variables[key]["entry"]], rows=[i + 1], cols=[1], sticky=["ew"]
            )
            if "lab" in variables[key].keys():
                self.add_to_grid(
                    widg=[variables[key]["lab"]], rows=[i + 1], cols=[0], sticky=["ew"]
                )
            elif "butt" in variables[key].keys():
                self.add_to_grid(
                    widg=[variables[key]["butt"]], rows=[i + 1], cols=[0], sticky=["ew"]
                )
    ...
    def draw_image(self, curr_fig=None, create_zoom_butt=True):
        ...
        if create_zoom_butt:
            ...
            self.variable_containers["duplicate"]["butt"].pack()
            ...
            self.variable_containers["set region"]["butt"].pack()
    ...
    def start_scan_widget(self, anal_frm, conf_frm):
        ...
        butt_close = ttk.Button(
            conf_frm, text="Close stream", command=press_close_stream
        )  # , activeforeground='blue', highlightbackground=self.button_color)
        butt_anal = ttk.Button(
            anal_frm, text="Analyze", command=lambda: press_analyze(from_file=True)
        )  # , activeforeground='blue', highlightbackground=self.button_color)

        butt_start.grid(row=50, column=1, columnspan=5, sticky="nsew", padx=0, pady=1.5)
        butt_cancel.grid(
            row=50, column=6, columnspan=4, sticky="nsew", padx=0, pady=1.5
        )
        butt_close.grid(row=50, column=0, sticky="nsew", padx=0, pady=1.5)
        butt_anal.grid(row=50, column=1, sticky="nsew", padx=0, pady=1.5)
    ...
    def move_with_plot_widget(self, tab):
        ...
        def press_moveto():
            self.t7.move_to_pos(x_coord=x_move_var.get(), y_coord=y_move_var.get())
        ...
        # ------
        x_move_var = tk.IntVar(value=1)
        y_move_var = tk.IntVar(value=1)

        # TODO: hide these until we plot
        move_lab = ttk.Label(frm_move, text=f"Move to pixels (x, y):")
        x_move_entry = ttk.Entry(frm_move, textvariable=x_move_var, width=10)
        y_move_entry = ttk.Entry(frm_move, textvariable=y_move_var, width=10)
        btn_move = ttk.Button(
    ...
    def counts_widget(self, tab):
        ...
        ttk.Label(counts_frame, text=f"Channel", width=5).grid(
    ...
    def log_scan_widget(self, tab):
        ...
        self.counts_widget(frm_log).grid(
            row=0, column=0, rowspan=1, sticky="news", padx=0, pady=0
        )  # in sub frame

        frm_logbox = ttk.Frame(frm_log, relief=tk.RAISED)
        ttk.Label(frm_logbox, text=f"Log", font=("", 15)).grid(
            row=0, column=0, sticky="w"
        )
        frm_logbox.grid(row=2, column=0, sticky="news", padx=0, pady=0)  # in sub frame
    ...
    def press_getzoomregion(
        ...
        if True:
            ...
            y_max, y_min = new_st.curr_fig.get_axes()[
                0
            ].get_ylim()  # NOTE:IMSHOW WILL HAVE Y AXIS FLIPPED
            print(
            ...
            if dup:
                ...
                self.curr_fig.get_axes()[0].set_ylim(y_max, y_min)  # NOTE FLIPPED AXIS
                self.curr_canvas.draw()

            new_st.curr_fig.get_axes()[0].set_xlim(x_min, x_max)
            new_st.curr_fig.get_axes()[0].set_ylim(y_max, y_min)  # NOTE FLIPPED AXIS
            ...
            new_st.t7.find_zoom_range(x_min, x_max, y_min, y_max)  # try to get voltages
            new_st.select_speed()
...
class WindowGUI:
    ...
    def init_window():
        try:
            ...
            root = ThemedTk(theme="breeze")  # 'yaru' , 'radiance', 'breeze'
        except:
...
class T7:
    ...
    def find_zoom_range(self, x_min, x_max, y_min, y_max):
        ...
        try:
            ...
            print(
                "x vals list:", x_vals[:10], "...\n\ny vals list:", y_vals[:10], "..."
            )
            """if self.speed_mode in ['fast', 'slow']:
        ...
        if abort_move and not self.gui.demo_mode.get():
            ...
            self.gui.variable_containers["amp X"]["var"].set(
                (x_pos_max - x_pos_min) / 2
            )
            self.gui.variable_containers["amp Y"]["var"].set(
                (y_pos_max - y_pos_min) / 2
            )
        else:
            print("ELSE")
            input("waiting....")
    ...
    def slow_galvo_scan(self, do_scan=True, ac_parameters = None):
        ...
        def create_sweep_lists(dir):
            ...
            for sweep_idx in range(len(sweep_vals_curr)):
                ...
                sweep_addresses_list_curr += [self.sine_addr]
        ...
        def do_scan_si():
            ...
            for step_idx in range(len(up_step_values)):
                ...
                if not self.offline:  # ****
                    ...
                    print(
                        f"({step_idx}) Step to new row: {[self.step_addr]} --> {[up_step_values[step_idx]]}"
                    )

                    if step_idx == 0:  # left sweep (every even row)
                        print(
                            f"-- {step_idx}) Sweep new left sweep row:\n"
                            f"Add:{sweep_addresses_cmds_left[:13]}\n"
                            f"Val:{sweep_values_cmds_left[:13]}"
                        )
                    elif step_idx == 1:  # right sweep (every odd row)
                        print(
                            f"-- {step_idx}) Sweep new right sweep row:\n "
                            f"   Addresses:{sweep_addresses_cmds_right[:13]}\n"
                            f"   Values:{sweep_values_cmds_right[:13]}"
                        )
            ...
            if not self.offline:  # ****
                ...
                self.logger_box.module_logger.info("Stopping server")
                time.sleep(1)
        ...
        try:
            ...
            if (
                self.abort_scan or not do_scan
            ):  # or self.offline:  # last line of defense
                self.logger_box.module_logger.info(
                    "Aborted scan (error or offline mode)"
                )
    ...
    def socket_connection(self, shutdown_server=False, doneScan=False):
        ...
        while run_flag:  # Keep looking for a connection
            ...
            if clientsocket:
                time.sleep(5)  # Give the qutag a few seconds to start up
                break
    ...
    def close_labjack_connection(self, printlog=True):
        ...
        if self.handle is None:
            ...
            time.sleep(
                1
            )  # probably don't need, just in case there is still some data being transmitted
